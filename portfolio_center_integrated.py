...
def render():
    ...
    st.caption("v1.2 RC Integration 2")
    ...
    if "return_pct" in df.columns:
        ...
        with a:
            ...
            st.dataframe(winners[_select_columns(winners)] if not winners.empty and _select_columns(winners) else winners, use_container_width=True)
        with b:
            st.subheader("⚠️ Under Pressure")
            weak = temp[temp["_return_pct"] <= -20]
            st.dataframe(weak[_select_columns(weak)] if not weak.empty and _select_columns(weak) else weak, use_container_width=True)
    ...
    if "Live Action 即時行動" in df.columns:
        ...
        st.dataframe(watch[_select_columns(watch)] if not watch.empty and _select_columns(watch) else watch, use_container_width=True)
